...
def normalize_99acres():
    norm = Normalizer()
    input_file = "99acres_test.json"
    output_file = "99acres_normalized.json"
    
    if not os.path.exists(input_file):
        print(f"Error: {input_file} not found.")
        return

    print(f"Normalizing {input_file} (99acres)...")
    ...
    for item in items:
        # 99acres usually specifies rent or sale in the title/propertyType
        intent = "rent"
        if "sale" in str(item.get("propertyType", "")).lower() or "sale" in str(item.get("title", "")).lower():
            intent = "sell"
        
        try:
            result = norm.normalize(item, source="99acres", property_type="residential", intent=intent)
            if result:
